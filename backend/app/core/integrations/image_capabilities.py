...
def register_image_model_capability(
    *,
    ...
    if provider == "openai":
        ...
        return
    if provider == "apimart":
        from app.core.integrations.apimart.image_capabilities import register_apimart_image_capability

        register_apimart_image_capability(model_prefix=model_prefix, capability=capability)
        return
...
def clear_image_model_capability_overrides(*, provider: ProviderKey | None = None) -> None:
    ...
    if provider is None:
        ...
        from app.core.integrations.apimart.image_capabilities import clear_apimart_image_capability_overrides

        clear_apimart_image_capability_overrides()
        return
    if provider == "openai":
        clear_openai_image_capability_overrides()
        return
...
def resolve_image_capability(*, provider: ProviderKey, model: str | None) -> ImageModelCapability:
    ...
    if provider == "apimart":
        from app.core.integrations.apimart.image_capabilities import resolve_apimart_image_capability

        return resolve_apimart_image_capability(model)
